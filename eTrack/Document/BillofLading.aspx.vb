...
Partial Class BillofLading
    ...
    Protected Overrides Function BindSourceData(ByVal intUserId As String, ByVal intPage As Integer, ByVal intSize As Integer) As Boolean
        ...
        Dim strSess As String = Session("hidVal_BillofLading")
        ...
        If strSess = 1 Then
            ...
            End If
            If Not Session("Condition1") Is Nothing Then
                'objList.Where.Replace(" " + Session("Condition1"), "")
                'objList.Where.Replace(" " + Session("Condition2"), "")
                objList.Where += " " + Session("Condition1")
            End If
            objList.Query = "Sebl1"
            objList.GetListInfo(intPage, intSize)
            objColumns = DynamicGridViewFun.GetColumnFromSqlFile("Sebl1", gvwSource, TemplateType.ListPrint, intUserId, Session("LoginType").ToString)
        ElseIf strSess = 2 Then
            If Session("LoginType") = 0 Then
                objList.Where += "  and customercode='" + objUser.UserId + "' "
            End If
            If Not Session("Condition2") Is Nothing Then
                'objList.Where.Replace(" " + Session("Condition1"), "")
                'objList.Where.Replace(" " + Session("Condition2"), "")
                objList.Where += " " + Session("Condition2")
            End If
            objList.Query = "Sibl1_BillofLading"
            objList.GetListInfo(intPage, intSize)
            objColumns = DynamicGridViewFun.GetColumnFromSqlFile("Sibl1_BillofLading", gvwSource, TemplateType.ListPrint, intUserId, Session("LoginType").ToString)
        End If
    ...
    Protected Overrides Function NewObjectList(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String) As clsBaseSrv
        Return New clsBillofLading(strUserId, RoleName, strFunNo)
    End Function
    ...
    Private Sub InitSearchListField()
        ...
        " from GridSet_Person b  where sTableName='Sebl1' and bFieldVisible=1 order by sEnglishTitle"
        ...
        " ,(select top 1 sFieldType from GridSet a where a.sTableName=b.sTableName) as sFieldType " & _
        " from GridSet_Person b  where sTableName='Sibl1_BillofLading' and bFieldVisible=1 order by sEnglishTitle "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        If Not IsPostBack Then
            ...
            Dim tabName As String() = {"Sebl1", "Sibl1_BillofLading"}
            For i As Integer = 0 To dtRight.Rows.Count - 1
                Dim RightNo As String = dtRight.Rows(i)(0).ToString
                Select Case RightNo
                    Case "4"
                        hidArr(0) = False
                        Session("Condition1") = dtRight.Rows(i)(1).ToString
                    Case "5"
                        hidArr(1) = False
                        Session("Condition2") = dtRight.Rows(i)(1).ToString
                End Select
            Next
            ...
            For i As Integer = 0 To hidArr.Length - 1
                If hidArr(i) = False Then
                    ...
                    If FirstTabFlag = False Then
                        ...
                        Session("hidVal_BillofLading") = (i + 1).ToString
    ...
    Public Function SetTabVal(ByVal hidVal As String) As String
        ...
        Session("hidVal_BillofLading") = hidVal
        hidVal_BillofLading.Value = hidVal
        Return ZZMessage.ConMsgRtn.Ok + ConSeparator.Par + GridViewFun.RenderControl(hidVal_BillofLading)
    End Function
    Protected Sub gvwSource_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            strTrxNo = CType(tmpProp, clsPropBillofLading).TrxNo
            'Popupmenu
            If strTrxNo > "" Then
            ...
            If objList.Query = "Sebl1" Then
                'imgPrint.Attributes.Add("OnClick", "PrintCrystalReport('rptsebl15a.rpt','{vw_Sebl1.Trx No}','" + strTrxNo + "','Int');return false;")
                imgPrint.Attributes.Add("OnClick", "PrintDetail(" + strTrxNo + ",'" + hidReports.Value + "');return false;")
            ElseIf objList.Query = "Sibl1_BillofLading" Then
                ' imgPrint.Attributes.Add("OnClick", "PrintCrystalReport('rptSibl02.rpt','{vw_Sibl1.Trx No}','" + strTrxNo + "','Int');return false;")
                imgPrint.Attributes.Add("OnClick", "PrintDetail2(" + strTrxNo + ",'" + hidReports2.Value + "');return false;")
            End If
    ...
    Private Sub BindAttach(ByVal intUserId As String, ByVal intTrxNo As Int64)
        ...
        objAttach.GetListInfo(Server.MapPath("..\Report\Documents\sebl\"), "")
        ...
        objAttach = New clsAttach(intUserId, intTrxNo)
        objAttach.GetListInfo(Server.MapPath("..\Report\Documents\sibl\"), "")
        arr = objAttach.ArrProp
        If arr.Count = 2 Then
            Dim tmpProp As clsProp = arr(0)
            hidReports2.Value += CType(tmpProp, clsPropAttach).FileName.Replace(".rpt", "")
        Else
            For i As Integer = 0 To arr.Count - 2
                Dim tmpProp As clsProp = arr(i)
                hidReports2.Value += CType(tmpProp, clsPropAttach).FileName.Replace(".rpt", "") + ","
