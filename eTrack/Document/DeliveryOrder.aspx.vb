...
Partial Class DeliveryOrder
    ...
    Protected Overrides Function BindSourceData(ByVal intUserId As String, ByVal intPage As Integer, ByVal intSize As Integer) As Boolean
        ...
        Dim strSess As String = Session("hidVal_DeliveryOrder")
        If strSess = 1 Then
            If Session("LoginType") = 0 Then
                objList.Where += "and ( jobno in (select jobno from sebl1 where customercode ='" + objUser.UserId + "') or jobno in (select jobno from sibl1 where customercode ='" + objUser.UserId + "'))  "
            End If
            objList.Query = "sido1"
            objList.GetListInfo(intPage, intSize)
            objColumns = DynamicGridViewFun.GetColumnFromSqlFile("sido1", gvwSource, TemplateType.ListPrint, intUserId, Session("LoginType").ToString)
        ElseIf strSess = 2 Then
            If Session("LoginType") = 0 Then
                objList.Where += "and  jobno in (select jobno from aiaw1 where customercode ='" + objUser.UserId + "')"
            End If
            objList.Query = "aido1"
            objList.GetListInfo(intPage, intSize)
            objColumns = DynamicGridViewFun.GetColumnFromSqlFile("aido1", gvwSource, TemplateType.ListPrint, intUserId, Session("LoginType").ToString)
        End If
    ...
    Protected Overrides Function NewObjectList(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String) As clsBaseSrv
        Return New clsDeliveryOrder(strUserId, RoleName, strFunNo)
    End Function
    ...
    Private Sub InitSearchListField()
        ...
        " from GridSet_Person b  where sTableName='sido1' and bFieldVisible=1 order by sEnglishTitle"
        ...
        " ,(select top 1 sFieldType from GridSet a where a.sTableName=b.sTableName) as sFieldType " & _
        " from GridSet_Person b  where sTableName='aido1' and bFieldVisible=1 order by sEnglishTitle "
        ...
        If dt1.Rows.Count <= 0 Then
            strSql1 = " select sFieldName,sEnglishTitle,sFieldType from GridSet where sTableName='sido1' order by sEnglishTitle"
            dt1 = BaseSelectSrvr.GetData(strSql1, "")
        End If
        ...
        If dt2.Rows.Count <= 0 Then
            strSql2 = " select sFieldName,sEnglishTitle,sFieldType from GridSet where sTableName='aido1' order by sEnglishTitle"
            dt2 = BaseSelectSrvr.GetData(strSql2, "")
        End If
...
objList = NewObjectList(objUser.UserId, objUser.RoleName, PageFun.GetFunNo(Page))
            Session("hidVal_DeliveryOrder") = "1"
            ...
            Dim tabName As String() = {"sido1", "aido1"}
            ...
            For i As Integer = 0 To hidArr.Length - 1
                If hidArr(i) = False Then
                    ...
                    If FirstTabFlag = False Then
                        ...
                        Session("hidVal_DeliveryOrder") = (i + 1).ToString
                        hidVal_DeliveryOrder.Value = (i + 1).ToString
                        If i = 1 Then
                            Me.TableName = "aido1"
                            Me.CurrentSortField = "DeliveryOrderNo"
                            objList.OrderBy = "DeliveryOrderNo Desc"
                        End If
    ...
    Public Function SetTabVal(ByVal hidVal As String) As String
        ...
        Session("hidVal_DeliveryOrder") = hidVal
        hidVal_DeliveryOrder.Value = hidVal
        Return ZZMessage.ConMsgRtn.Ok + ConSeparator.Par + GridViewFun.RenderControl(hidVal_DeliveryOrder)
    End Function
    ...
    Protected Sub gvwSource_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If objList.Query = "sido1" Then
                strTrxNo = CType(tmpProp, clsPropDeliveryOrder).TrxNo
            Else
                strTrxNo = CType(tmpProp, clsPropDeliveryOrder).DeliveryOrderNo
            End If
            'Popupmenu
            If strTrxNo > "" Then
            ...
            If objList.Query = "sido1" Then
                'imgPrint.Attributes.Add("OnClick", "PrintDetail('DeliveryOrder','','{vw_sido1.Trx No}','" + strTrxNo + "','Documents\\rptSido13.rpt');return false;")
                imgPrint.Attributes.Add("OnClick", "PrintCrystalReport('rptSido13.rpt','{vw_sido1.Trx No}','" + strTrxNo + "','Int');return false;")
            ElseIf objList.Query = "aido1" Then
                'imgPrint.Attributes.Add("OnClick", "PrintDetail('DeliveryOrder','','{vw_aido1.Delivery Order No}','" + strTrxNo + "','Documents\\rptAido11.rpt');return false;")
                imgPrint.Attributes.Add("OnClick", "PrintCrystalReport('rptAido11.rpt','{vw_aido1.Delivery Order No}','" + strTrxNo + "','Char');return false;")
            End If
