...
Partial Class JobAnalysis
    ...
    Protected Overrides Function NewObjectList(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String) As clsBaseSrv
        Return New clsJobAnalysis(strUserId, RoleName, strFunNo)
    End Function
    Protected Overrides Sub OnInit(ByVal e As System.EventArgs)
        ...
        drType.Items.Add(New ListItem("Shipment", "Shipment"))
        drType.Items.Add(New ListItem("Tonnage", "Tonnage"))
        drType.Items.Add(New ListItem("Sales", "Sales"))
        drType.Items.Add(New ListItem("Profit", "Profit"))
    ...
    Private Sub InitSearchListField()
        ...
        drMonthTo.Text = strM
        'Field 
        Dim theSearch As New ArrayList
        theSearch.Add("Coloader")
        theSearch.Add("Consignee")
        theSearch.Add("Customer")
        theSearch.Add("OverSeas Agent")
        Dim strSess As String = Session("hidVal_JobAnalysis")
        If strSess = 3 Or strSess = 4 Then
            '《For Sea
            theSearch.Add("Port of Discharge")
            theSearch.Add("Port of Loading")
            'For Sea》
        Else
            theSearch.Add("Airport of Origin")
            theSearch.Add("Airport of Dest")
        End If
        theSearch.Add("Salesman")
        theSearch.Add("Shipper")
        If strSess = 3 Or strSess = 4 Then
            '《For Sea
            theSearch.Add("Shippingline")
            'For Sea》
        Else
            theSearch.Add("Airline")
        End If
        drpSearch.DataSource = theSearch
        drpSearch.DataBind()


        drhard.DataSource = theSearch
        drhard.DataBind()
...
objList = NewObjectList(objUser.UserId, objUser.RoleName, PageFun.GetFunNo(Page))
    ...
    Public Function SetTabVal(ByVal hidVal As String) As String
        ...
        Session("hidVal_JobAnalysis") = hidVal
        InitSearchListField()
        hidVal_JobAnalysis.Value = hidVal
        Return ZZMessage.ConMsgRtn.Ok + ConSeparator.Par + GridViewFun.RenderControl(hidVal_JobAnalysis) + ConSeparator.Par + GridViewFun.RenderControl(drpSearch)
    End Function
    Protected Sub gvwSource_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            strAwbNo = CType(tmpProp, clsPropJobAnalysis).AwbNo
            'Popupmenu
            If strAwbNo > "" Then
