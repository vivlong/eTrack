...
Partial Class ContainerMaster
    ...
    Protected Overrides Function NewObjectList(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String) As clsBaseSrv
        Return New clsContainerMaster(strUserId, RoleName, strFunNo)
    End Function
...
objList = NewObjectList(objUser.UserId, objUser.RoleName, PageFun.GetFunNo(Page))
            ...
            'btnPrint.Attributes.Add("OnClick", ConPrintLimit.PrintButtonOnClick(objUser.PagePara.QuerySize, GetType(clsTankDetailReport).AssemblyQualifiedName, TableName, 3, True))
    ...
    Protected Sub gvwSource_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            'KeyValue 
            Dim intTrxNo As String = CType(tmpProp, clsPropContainerMaster).TrxNo.ToString()
            'Popupmenu
            If intTrxNo > 0 Then
                e.Row.Attributes.Add("oncontextmenu", "ShowMenu(" + intTrxNo.ToString() + ");return false;")
            End If
            'Row color
