<%@ Page Language="VB" AutoEventWireup="true" CodeFile="SelectReportSebl_BL.aspx.vb"
    Inherits="SelectReportSebl_BL" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        function PrintDetail(intId) {
            ...
            if (strTrxNo != -1) {
                var strUrl = "../loading.aspx?tourl=../Print/crSebl_BL.aspx?Id=" + intId.toString() + "";
                window.open(strUrl);
            }
        }
        function OpenReport(objList, id) {
            var strReport = objList.options[objList.selectedIndex].value;
            if (strReport == "") { alert("please select a Report."); return false; }
            if (id != "") {
                var strUrl = "../loading.aspx?tourl=Print/crSebl_BL.aspx?Id=" + id.toString() + "&Report=" + strReport;
                window.open(strUrl);
                this.close();
            }
        }
...
<body>
    ...
    <div id="divForm">
        <div id="divTopNav" style="width: 100%">
            <ul id="ulTopNav">
                <li>
                    <asp:Label ID="a3" CssClass="f12e navNml navOn" runat="server" Text="Select Vessel" /></li>
            </ul>
        </div>
