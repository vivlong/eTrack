...
#Region " Class Collection of CostEnquiry Info "

    <Serializable()> _
    Public Class colCostEnquiry
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropCostEnquiry(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of CostEnquiry Info "

    <Serializable()> _
    Public Class clsCostEnquiry
        Inherits clsBaseSrv
        ...
        Public Sub New(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String)
            ...
            Title = "CostEnquiry"
        End Sub
        Private Function InsertCostEnquiry(ByRef propCostEnquiry As clsPropCostEnquiry, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(51) As SqlParameter
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)

                param(51).Value = propCostEnquiry.CostEnquiryCode

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "", param)
                If intResult > 0 Then
                    propCostEnquiry.CostEnquiryCode = intResult
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdateCostEnquiry(ByVal propCostEnquiry As clsPropCostEnquiry, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
            ...
            End Try
        End Function
        Private Function DeleteCostEnquiry(ByVal propCostEnquiry As clsPropCostEnquiry, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(0) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.BigInt)
                param(0).Value = propCostEnquiry.CostEnquiryCode
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropCostEnquiry
                If tmpId < 0 Then
                    CurrentProp = colProp.NewOneProp()
                    tmpProp = CurrentProp
                    tmpProp.CostEnquiryCode = tmpId
                ...
                End If
                'tmpProp.CostEnquiryDate = GeneralFun.StringToDate(strRow(1))

                tmpProp.CostEnquiryCode = strRow(51)
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropCostEnquiry = CType(CurrentProp, clsPropCostEnquiry)
            blnReturn = InsertCostEnquiry(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveSuccess, Title, tmpProp.CostEnquiryCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveFailure, Title, tmpProp.CostEnquiryCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropCostEnquiry = CType(CurrentProp, clsPropCostEnquiry)
            blnReturn = UpdateCostEnquiry(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateSuccess, Title, tmpProp.CostEnquiryCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateFailure, Title, tmpProp.CostEnquiryCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropCostEnquiry = CType(CurrentProp, clsPropCostEnquiry)
            blnReturn = DeleteCostEnquiry(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteSuccess, Title, tmpProp.CostEnquiryCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteFailure, Title, tmpProp.CostEnquiryCode)
                End If
                Return False
            End If
        End Function
