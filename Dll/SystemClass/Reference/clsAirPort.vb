...
#Region " Class Collection of AirPort Info "

    <Serializable()> _
    Public Class colAirPort
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropAirPort(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of AirPort Info "
    ...
    Public Class clsAirPort
        ...
        Public Sub New(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String)
            ...
            Title = "AirPort"
        End Sub
        Private Function InsertAirPort(ByRef propAirPort As clsPropAirPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(51) As SqlParameter
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)

                param(51).Value = propAirPort.AirPortCode

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "", param)
                If intResult > 0 Then
                    propAirPort.AirPortCode = intResult
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdateAirPort(ByVal propAirPort As clsPropAirPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
            ...
            End Try
        End Function
        Private Function DeleteAirPort(ByVal propAirPort As clsPropAirPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(0) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.BigInt)
                param(0).Value = propAirPort.AirPortCode
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_Track_Rcap1_List", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropAirPort
                If tmpId < 0 Then
                    CurrentProp = colProp.NewOneProp()
                    tmpProp = CurrentProp
                    tmpProp.AirPortCode = tmpId
                ...
                End If
                'tmpProp.AirPortDate = GeneralFun.StringToDate(strRow(1))

                tmpProp.AirPortCode = strRow(51)
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropAirPort = CType(CurrentProp, clsPropAirPort)
            blnReturn = InsertAirPort(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveSuccess, Title, tmpProp.AirPortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveFailure, Title, tmpProp.AirPortCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropAirPort = CType(CurrentProp, clsPropAirPort)
            blnReturn = UpdateAirPort(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateSuccess, Title, tmpProp.AirPortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateFailure, Title, tmpProp.AirPortCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropAirPort = CType(CurrentProp, clsPropAirPort)
            blnReturn = DeleteAirPort(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteSuccess, Title, tmpProp.AirPortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteFailure, Title, tmpProp.AirPortCode)
                End If
                Return False
            End If
        End Function
