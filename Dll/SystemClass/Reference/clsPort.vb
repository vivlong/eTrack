...
#Region " Class Collection of Port Info "

    <Serializable()> _
    Public Class colPort
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropPort(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of Port Info "
    ...
    Public Class clsPort
        ...
        Public Sub New(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String)
            ...
            Title = "Port"
        End Sub
        Private Function InsertPort(ByRef propPort As clsPropPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(51) As SqlParameter
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)

                param(51).Value = propPort.PortCode

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "", param)
                If intResult > 0 Then
                    propPort.PortCode = intResult
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdatePort(ByVal propPort As clsPropPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
            ...
            End Try
        End Function
        Private Function DeletePort(ByVal propPort As clsPropPort, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(0) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.BigInt)
                param(0).Value = propPort.PortCode
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_Track_Rcsp1_List", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropPort
                If tmpId < 0 Then
                    CurrentProp = colProp.NewOneProp()
                    tmpProp = CurrentProp
                    tmpProp.PortCode = tmpId
                ...
                End If
                'tmpProp.PortDate = GeneralFun.StringToDate(strRow(1))

                tmpProp.PortCode = strRow(51)
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropPort = CType(CurrentProp, clsPropPort)
            blnReturn = InsertPort(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveSuccess, Title, tmpProp.PortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveFailure, Title, tmpProp.PortCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropPort = CType(CurrentProp, clsPropPort)
            blnReturn = UpdatePort(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateSuccess, Title, tmpProp.PortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateFailure, Title, tmpProp.PortCode)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropPort = CType(CurrentProp, clsPropPort)
            blnReturn = DeletePort(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteSuccess, Title, tmpProp.PortCode)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteFailure, Title, tmpProp.PortCode)
                End If
                Return False
            End If
        End Function
