...
#Region " Class Collection of SO2 Info "
    ...
    Public Class colSO2
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropSO2(ConClass.NewIdValue)
        End Function
    End Class
#End Region
#Region " Class Server of SO2 Info "
    ...
    Public Class clsSO2
        ...
        Private Function InsertSO2(ByRef propSO As clsPropSO2, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(23) As SqlParameter

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spi_ctso2", param)
                If intResult > 0 Then
                    propSO.TrxNo = Int64.Parse(param(23).Value.ToString())
            ...
            End Try
        End Function
        Private Function UpdateSO2(ByVal propSO As clsPropSO2, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(15) As SqlParameter


                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spu_ctso2", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
            ...
            End Try
        End Function
        Private Function DeleteSO2(ByVal propSO2 As clsPropSO2, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(1) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.BigInt)
                param(0).Value = propSO2.TrxNo
                param(1) = New SqlParameter("@LineItemNo", SqlDbType.BigInt)
                param(1).Value = propSO2.LineItemNo

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spd_ctso2", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                If strType = "ReleaseContainer" Then
                    ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_ctso2", param)
                Else
                    ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_ctso2_List", param)
                End If
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
        ...
        Public Overrides Function GetDetail(ByVal intId As Long) As clsProp
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_ctso2_Detail", param)
                dt = ds.Tables(0)
                Return colProp.RecordToProp(dt, 0, 0)
            Catch
                Return colProp.NewOneProp()
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropSO2 = CType(CurrentProp, clsPropSO2)
            blnReturn = InsertSO2(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropSO2 = CType(CurrentProp, clsPropSO2)
            blnReturn = UpdateSO2(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropSO2 = CType(CurrentProp, clsPropSO2)
            blnReturn = DeleteSO2(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
