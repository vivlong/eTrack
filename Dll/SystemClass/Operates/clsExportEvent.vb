...
#Region " Class Collection of ExportEvent Info "

    <Serializable()> _
    Public Class colExportEvent
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropExportEvent(ConClass.NewIdValue)
        End Function
    End Class
#End Region
#Region " Class Server of ExportEvent Info "

    <Serializable()> _
    Public Class clsExportEvent
        Inherits clsBaseSrv
        ...
        Private Function DeleteExportEvent(ByVal propExportEvent As clsPropExportEvent, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(0) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.BigInt)
                param(0).Value = propExportEvent.TrxNo

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spd_ExportEvent1", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_ctcl1_ExportEvent_List", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function GetDetail(ByVal intId As Long) As clsProp
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_ctcl1_ExportEvent1_Detail", param)
                dt = ds.Tables(0)
                Return colProp.RecordToProp(dt, 0, 0)
            Catch
                Return colProp.NewOneProp()
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropExportEvent
                If tmpId < 0 Then
        ...
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropExportEvent = CType(CurrentProp, clsPropExportEvent)
            blnReturn = DeleteExportEvent(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
