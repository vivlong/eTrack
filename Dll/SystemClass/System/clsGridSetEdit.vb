...
#Region " Class Collection of GridSet "

    <Serializable()> _
    Public Class colGridSetEdit
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropGridSetEdit(ConClass.NewIdValue)
        End Function
    End Class
#End Region
#Region " Class Server of GridSet "

    <Serializable()> _
    Public Class clsGridSetEdit
        Inherits clsBaseSrv
        ...
        Public Sub New(ByVal intUserId As String)
            ...
            _ConfirmExternal = False
            colProp = New colGridSet()
            Title = "GridSet"
            colProp.AddOneSpaceRecord(0, intUserId)
        End Sub
        Private Function InsertGridSet(ByRef propFI As clsPropGridSetEdit, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(23) As SqlParameter

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spi_Track_GridSet", param)
                If intResult > 0 Then
                    propFI.sTableName = Int64.Parse(param(23).Value.ToString())
            ...
            End Try
        End Function
        Private Function UpdateGridSet(ByVal propFI As clsPropGridSetEdit, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(15) As SqlParameter


                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spu_Track_GridSet", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
            ...
            End Try
        End Function
        Private Function DeleteGridSet(ByVal propFI As clsPropGridSetEdit, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(1) As SqlParameter

                param(0) = New SqlParameter("@sTableName", SqlDbType.NVarChar, 50)
                param(0).Value = propFI.sTableName

                param(1) = New SqlParameter("@sFieldName", SqlDbType.NVarChar, 50)
                param(1).Value = propFI.sFieldName

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spd_Track_GridSet", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_Track_GridSet_Column_List", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function GetDetail(ByVal intId As Long) As clsProp
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_eTrack_GridSet_Detail", param)
                dt = ds.Tables(0)
                Return colProp.RecordToProp(dt, 0, 0)
            Catch
                Return colProp.NewOneProp()
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropGridSetEdit = CType(CurrentProp, clsPropGridSetEdit)
            blnReturn = InsertGridSet(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveSuccess, Title, tmpProp.sTableName)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveFailure, Title, tmpProp.sTableName)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropGridSetEdit = CType(CurrentProp, clsPropGridSetEdit)
            blnReturn = UpdateGridSet(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateSuccess, Title, tmpProp.sTableName)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateFailure, Title, tmpProp.sTableName)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropGridSetEdit = CType(CurrentProp, clsPropGridSetEdit)
            blnReturn = DeleteGridSet(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteSuccess, Title, tmpProp.sTableName)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteFailure, Title, tmpProp.sTableName)
                End If
                Return False
            End If
        End Function
