...
#Region " Class Collection of Manifest Info "

    <Serializable()> _
    Public Class colManifest
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropManifest(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of Manifest Info "

    <Serializable()> _
    Public Class clsManifest
        Inherits clsBaseSrv
        ...
        Private Function InsertManifest(ByRef propManifest As clsPropManifest, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(51) As SqlParameter
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)

                param(51).Value = propManifest.AwbNo

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "", param)
                If intResult > 0 Then
                    propManifest.AwbNo = intResult
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdateManifest(ByVal propManifest As clsPropManifest, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
            ...
            End Try
        End Function
        Private Function DeleteManifest(ByVal propManifest As clsPropManifest, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                Dim param(0) As SqlParameter

                param(0) = New SqlParameter("@AwbNo", SqlDbType.BigInt)
                param(0).Value = propManifest.AwbNo
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropManifest
                If tmpId < 0 Then
                ...
                End If
                'tmpProp.ManifestDate = GeneralFun.StringToDate(strRow(1))
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropManifest = CType(CurrentProp, clsPropManifest)
            blnReturn = InsertManifest(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropManifest = CType(CurrentProp, clsPropManifest)
            blnReturn = UpdateManifest(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropManifest = CType(CurrentProp, clsPropManifest)
            blnReturn = DeleteManifest(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
