...
#Region " Class Collection of Event Info "
    ...
    Public Class colEvent
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropEvent(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of Event Info "
    ...
    Public Class clsEvent
        ...
        Public Sub New(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String)
            ...
            Title = "Event"
        End Sub
        Private Function InsertEvent(ByRef propEvent As clsPropEvent, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(3) As SqlParameter

                param(0) = New SqlParameter("@TrxNo", SqlDbType.Int)
                param(1) = New SqlParameter("@EventDate", SqlDbType.DateTime)
                param(2) = New SqlParameter("@EventTitle", SqlDbType.NVarChar, 50)
                param(3) = New SqlParameter("@Comments", SqlDbType.NVarChar, 80)

                param(0).Value = propEvent.TrxNo
                param(1).Value = propEvent.EventDate
                param(2).Value = propEvent.EventTitle
                param(3).Value = propEvent.Comments

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spi_Track_Popo3", param)
                If intResult > 0 Then
                    propEvent.TrxNo = intResult
                    masterId = propEvent.TrxNo
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdateEvent(ByVal propEvent As clsPropEvent, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(4) As SqlParameter
                param(0) = New SqlParameter("@TrxNo", SqlDbType.Int)
                param(1) = New SqlParameter("@LineItemNo", SqlDbType.Int)
                param(2) = New SqlParameter("@EventDate", SqlDbType.DateTime)
                param(3) = New SqlParameter("@EventTitle", SqlDbType.NVarChar, 50)
                param(4) = New SqlParameter("@Comments", SqlDbType.NVarChar, 80)

                param(0).Value = propEvent.TrxNo
                param(1).Value = propEvent.LineItemNo
                param(2).Value = propEvent.EventDate
                param(3).Value = propEvent.EventTitle
                param(4).Value = propEvent.Comments
                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spu_Track_Popo3", param)
                If intResult > 0 Then
                    masterId = propEvent.TrxNo
                    Return True
                Else
            ...
            End Try
        End Function
        Private Function DeleteEvent(ByVal propEvent As clsPropEvent, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
                ...
                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spd_Track_Popo3", param)
                If intResult > 0 Then
                    Return True
                Else
                    Return False
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_Track_Popo3_List", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function GetDetail(ByVal intId As Long) As clsProp
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_Track_Popo3_Detail", param)
                dt = ds.Tables(0)
        ...
        Public Overrides Function GetDetail(ByVal strId As String) As clsProp
            Dim ds As DataSet
            Dim dt As DataTable
            Dim intTrxNo As Integer = -1
            Dim intLineItemNo As Integer = -1
            Dim arrId As String() = strId.Split("&")
            If arrId.Length > 0 Then
                intTrxNo = arrId(0)
                intLineItemNo = arrId(1)
            End If
            Try
                Dim param(1) As SqlParameter
                param(0) = New SqlParameter("@TrxNo", SqlDbType.Int)
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            If strRow.Length <> 6 Then
                Return False
            Else
                Dim tmpId As Int64 = Int64.Parse(strRow(5))
                Dim tmpProp As clsPropEvent
                If tmpId < 0 Then
                ...
                End If
                tmpProp.TrxNo = GeneralFun.StringToInt(strRow(0))
                tmpProp.EventDate = GeneralFun.StringToDate(strRow(1))
                tmpProp.EventTitle = strRow(2)
                tmpProp.Comments = strRow(3)
                tmpProp.LineItemNo = GeneralFun.StringToInt(strRow(5))
                CurrentProp = tmpProp
                Return True
            End If
        End Function
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropEvent = CType(CurrentProp, clsPropEvent)
            blnReturn = InsertEvent(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropEvent = CType(CurrentProp, clsPropEvent)
            blnReturn = UpdateEvent(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
        ...
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropEvent = CType(CurrentProp, clsPropEvent)
            blnReturn = DeleteEvent(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
