...
#Region " Class Collection of OperateRecord Info "

    <Serializable()> _
    Public Class colOperateRecord
        Inherits colclsProp
        ...
        Public Overrides Function NewOneProp() As clsProp
            Return New clsPropOperateRecord(ConClass.NewIdValue.ToString)
        End Function
    End Class
#End Region
#Region " Class Server of OperateRecord Info "

    <Serializable()> _
    Public Class clsOperateRecord
        Inherits clsBaseSrv
        ...
        Public Sub New(ByVal strUserId As String, ByVal RoleName As String, ByVal strFunNo As String)
            ...
            Title = "OperateRecord"
        End Sub
        Private Function InsertOperateRecord(ByRef propOperateRecord As clsPropOperateRecord, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(25) As SqlParameter
                param(0) = New SqlParameter("@LineItemNo", SqlDbType.NVarChar, 50)

                param(25) = New SqlParameter("@id", SqlDbType.Int)
                param(25).Direction = ParameterDirection.Output

                param(0).Value = propOperateRecord.Id
                param(25).Value = propOperateRecord.Id

                Dim intResult As Integer
                intResult = SqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, "spi_Track_Popo2", param)
                If intResult > 0 Then
                    propOperateRecord.Id = intResult
                    Return True
                Else
                    Return False
                End If
            ...
            End Try
        End Function
        Private Function UpdateOperateRecord(ByVal propOperateRecord As clsPropOperateRecord, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Try
                Dim param(50) As SqlParameter
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)
                param(0) = New SqlParameter("@OrderDate", SqlDbType.NVarChar, 50)

                param(51).Value = propOperateRecord.Id
            ...
            End Try
        End Function
        Private Function DeleteOperateRecord(ByVal propOperateRecord As clsPropOperateRecord, ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByVal intDeleteType As Integer, ByRef strMsg As String) As Boolean
            Try
        ...
        Protected Overrides Function GetList() As DataTable
            ...
            Try
                ...
                ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.StoredProcedure, "sps_OperateRecord", param)
                If PageSize = 0 Then
                    'Total Page Count
                    PageCount = 1
                    'Total Record Count
        ...
        Public Overrides Function ModifyCurrent(ByVal strValue As String) As Boolean
            ...
            Else
                ...
                Dim tmpProp As clsPropOperateRecord
                If tmpId < 0 Then
        ...
        Protected Overrides Function InsertData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            ...
            Dim tmpProp As clsPropOperateRecord = CType(CurrentProp, clsPropOperateRecord)
            blnReturn = InsertOperateRecord(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveSuccess, Title, tmpProp.Id)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeySaveFailure, Title, tmpProp.Id)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function UpdateData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropOperateRecord = CType(CurrentProp, clsPropOperateRecord)
            blnReturn = UpdateOperateRecord(tmpProp, conn, trans, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateSuccess, Title, tmpProp.Id)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyUpdateFailure, Title, tmpProp.Id)
                End If
                Return False
            End If
        End Function
        Protected Overrides Function DeleteData(ByVal conn As SqlConnection, ByVal trans As SqlTransaction, ByRef strMsg As String) As Boolean
            Dim blnReturn As Boolean
            Dim tmpProp As clsPropOperateRecord = CType(CurrentProp, clsPropOperateRecord)
            blnReturn = DeleteOperateRecord(tmpProp, conn, trans, 1, strMsg)
            If blnReturn Then
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteSuccess, Title, tmpProp.Id)
                End If
                Return True
            Else
                If strMsg = "" Then
                    strMsg = ZZMessage.clsMessage.GetFormatMessage(ConMsgInfo.KeyDeleteFailure, Title, tmpProp.Id)
                End If
                Return False
            End If
        End Function
